...
-Como funciona:
	...
	Este projeto tem como objetivo a prática de uso de API's por meio de três exercícios, sendo este o primeiro 	deles (CEP, Temperatura e Livre). A proposta é apresentar a temperatura atual de qualquer cidade do Brasil. 	Por ser o primeiro exercício utilizando API com Javascript houve bastante dificuldade na compreensão de como 	se obter os resultados previstos. Para isso recorri a dois vídeos no YouTube(links em Referências). O 	primeiro vídeo foi utilizado mais para a compreensão de como utilizar a API no geral. O segundo foi o guia 	desse trabalho.
